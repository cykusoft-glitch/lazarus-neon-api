...
export default async function handler(req, res) {
    ...
    const { license_key, fp_primary, fp_secondary, device_label } = req.body;
    ...
    try {
        ...
        // Hashing Fingerprint untuk keamanan (menggunakan salt/pepper)
        const hashedFp = crypto.createHash('sha256').update(fp_primary + PEPPER).digest('hex');
        
        // Hashing fp_secondary jika ada, agar tetap aman dan tidak plain text di DB
        const hashedSecondary = fp_secondary 
            ? crypto.createHash('sha256').update(fp_secondary + PEPPER).digest('hex') 
            : null;

        // 2. Cek apakah device ini sudah pernah terdaftar
        ...
        await sql`
            INSERT INTO company_devices (license_key, fp_hash, fp_secondary_hash, device_label, status, activated_at)
            VALUES (${license_key}, ${hashedFp}, ${hashedSecondary}, ${device_label || 'Unlabeled Device'}, 'active', NOW())
        `;
